...
# Get the VM's external IP from GCP metadata
...
if [ "$ENABLE_HTTPS" = "true" ]; then
    ...
    email $EMAIL
        ...
        export SL_API_HTTP_FRONT_URL="https://$DOMAIN_NAME"
        export SL_API_DOMAIN="$DOMAIN_NAME"
    else
        CADDY_CONFIG=":80, :443 {
    reverse_proxy localhost:8080
}"
        export SL_API_HTTP_FRONT_URL="https://$EXTERNAL_IP"
        export SL_API_DOMAIN="$EXTERNAL_IP"
    fi
    ...
    export SL_PORT=80
    export SL_API_HTTP_FRONT_URL="http://$EXTERNAL_IP"
    export SL_API_DOMAIN="$EXTERNAL_IP"
fi
